...
public class scrMemOrb : MonoBehaviour
{
    ...
    void EndingSequence()
    {
        ...
        {
            Camera.main.orthographicSize *= shrinkRate;
            transform.localScale *= shrinkRate; 
            originalSize *= shrinkRate;
            if(transform.localScale.x<textTimeArr[0])
            {
                if(transform.localScale.x<textTimeArr[1])
                {
                    textObj.text = textArr[2];
                }
                else
        ...
        {
            textObj.color = new Color(0,0,0,0);
            objPlayer.GetComponent<scrPlayer>().lockMovement = false;
            if(Camera.main.orthographicSize < Camera.main.GetComponent<CameraFollow>().camSizeZoom)
                Camera.main.orthographicSize *= growthRate;
            else
            {
                Camera.main.orthographicSize = Camera.main.GetComponent<CameraFollow>().camSizeZoom;
